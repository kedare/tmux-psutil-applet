...
def check_disk_usage(warn: int = DISK_USAGE_WARN, crit: int = DISK_USAGE_CRIT) -> Status:
    ...
    disks_datas = [[disk.mountpoint, psutil.disk_usage(disk.mountpoint).percent] for disk in disks]
    disks_criticals = [disk_data for disk_data in disks_datas if disk_data[1] > crit]
    disks_warnings = [disk_data for disk_data in disks_datas if disk_data[1] > warn and disk_data[1] < crit]

    if len(disks_criticals) > 0:
        return Status(StatusCode.CRITICAL, "Disk {mount}: {usage}%".format(mount=disks_criticals[0][0], usage=disks_criticals[0][1]))
    elif len(disks_warnings) > 0:
        return Status(StatusCode.WARNING, "Disk {mount}: {usage}%".format(mount=disks_warnings[0][0], usage=disks_warnings[0][1]))
    else:
...
def main():
    ...
    # List of the checks by order of priority (the first check in error will hide the others until resolved, better to put the short errors first)
    check_definitions = [
        check_cpu_usage,
        check_memory_usage,
        check_disk_usage,
    ...
    print("#[fg=colour{bg_color},bg=colour238]#[fg=colour{fg_color},bg=colour{bg_color}] {text} #[fg=colour238,bg=colour{bg_color}]#[fg=colour255,bg=colour238]".format(bg_color=bg_color, fg_color=fg_color, text=text))
